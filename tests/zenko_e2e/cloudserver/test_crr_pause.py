...
def test_aws_1_1(aws_crr_bucket, aws_crr_bucket, testfile, objkey):
    ...
    assert util.check_object(objkey + '1',
        testfile, aws_crr_bucket, aws_crr_target_bucket, timeout=30)
    pause_url = conf.ZENKO_ENDPOINT + '/_/backbeat/api/crr/pause'
    response = requests.post(pause_url)
    # check reponse for Error
    aws_crr_bucket.put_object(
        Body=testfile,
        Key=objkey + '2'
    )
    assert util.check_object_dne(objkey + '2',
        testfile, aws_crr_bucket, aws_crr_target_bucket, timeout=30)
    resume_url = conf.ZENKO_ENDPOINT + '/_/backbeat/api/crr/resume'
    response = requests.post(resume_url)
    assert util.check_object(objkey + '2',
        testfile, aws_crr_bucket, aws_crr_target_bucket, timeout=30)
...
def test_gcp_1_1(gcp_crr_bucket, gcp_crr_target_bucket, testfile, objkey):
    ...
    assert util.check_object(objkey + '1',
        testfile, gcp_crr_bucket, gcp_crr_target_bucket, timeout=30)
    pause_url = conf.ZENKO_ENDPOINT + '/_/backbeat/api/crr/pause'
    response = requests.post(pause_url)
    # check reponse for Error
    gcp_crr_bucket.put_object(
...
def test_azure_1_1(azure_crr_bucket, azure_crr_target_bucket, testfile, objkey):
    ...
    assert util.check_object(objkey + '1',
        testfile, azure_crr_bucket, azure_crr_target_bucket, timeout=30)
    pause_url = conf.ZENKO_ENDPOINT + '/_/backbeat/api/crr/pause'
    response = requests.post(pause_url)
    # check reponse for Error
    azure_crr_bucket.put_object(
...
def test_wasabi_1_1(wasabi_crr_bucket, wasabi_crr_target_bucket, testfile, objkey):
    ...
    assert util.check_object(objkey + '1',
        testfile, wasabi_crr_bucket, wasabi_crr_target_bucket, timeout=30)
    pause_url = conf.ZENKO_ENDPOINT + '/_/backbeat/api/crr/pause'
    response = requests.post(pause_url)
    # check reponse for Error
    wasabi_crr_bucket.put_object(
...
@pytest.mark.conformance
def test_multi_1_M( multi_crr_bucket, aws_crr_target_bucket,
    ...
    assert util.check_object(objkey + '1', testfile, multi_crr_bucket,
        aws_crr_target_bucket, gcp_crr_target_bucket, azure_crr_target_bucket,
